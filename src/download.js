...
export const download = async function ({ version, output, arch, platform, fetchOpts }) {
  ...
  const nodePath = join(output, version, platform, archA, NODE_FILENAME)

  if (await pathExists(nodePath)) {
    return nodePath
  }

  await downloadFile({ version, nodePath, arch: archA, platform, fetchOpts })
...
const downloadFile = async function ({ version, nodePath, arch, platform, fetchOpts }) {
  ...
  try {
    await tmpDownload({ version, tmpFile, arch, platform, fetchOpts })
